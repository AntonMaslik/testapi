...
export class TaskController {
  ...
  @UseGuards(AccessTokenGuard)
  @Put(':id')
  updateTask(@Param() id: number, @Body() task: TaskUpdateRequestDto): Promise<UpdateResult> {
    return this.taskServices.updateTask(id, task);
  }

  @UseGuards(AccessTokenGuard)
  @Put('update-position')
  updateTaskPosition(@Body() { id, position }: TaskPositionUpdateDto): Promise<UpdateResult | NotFoundException> {
